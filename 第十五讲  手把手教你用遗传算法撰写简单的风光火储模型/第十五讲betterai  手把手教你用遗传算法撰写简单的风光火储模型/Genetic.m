%他有四个数据没给我看"电价.txt"，"典型日负荷.txt"，"PV.txt"，"WT.txt"
%数组对不上，不知道是哪里有的问题
%% 清空环境
clc
clear
...
for gen = 2:MAXGEN
    ...
    % 输出当前代的信息
    fprintf('代数: %d, 最佳适应度: %.4f\n', gen, min(individuals.fitness));
    [best_fitness, best_index] = min(individuals.fitness);
    best_chrom = individuals.chrom(best_index,:);
    fprintf('最佳个体部分基因值: ');
    fprintf('%.2f ', best_chrom(1:5));
    fprintf('...\n');
...
% 保存图像
saveas(figure(1), 'evolution_curve.png');
...
legend('储能容量变化')
    ...
    % 保存所有图像
    saveas(figure(1), 'evolution_curve1.png');
    saveas(figure(2), 'battery_state.png');
    saveas(figure(3), 'power_distribution.png');
