...
% 风-光-储—买卖电   因为这个程序有买卖电这个东西，所以就不会不收敛（应该是这样），所以程序就很好调教
...
%% 画图 - 所有图表显示在一张大图上
figure('Position',[100 100 1200 900]); % 创建一个大窗口
...
subplot(3,2,6);
s = value([Pload]);
plot(s,'r-*','LineWidth',2)  % 修正为plot(s)而不是plot(a)
legend('电负荷');
xlabel('时刻（t）');
ylabel('功率（kW）');
title('电负荷');
