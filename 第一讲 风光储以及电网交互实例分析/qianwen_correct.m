...
F_e = 0;
for i = 1:24
    F_e = F_e + 0.1*(P_ES1_cha(1,i)+P_ES1_dis(1,i))+0.6*Ppv(i)+0.5*Pwt(i)+e_p(i)*P_buy(i)-e_s.*P_sell(i);
end
F = F_e;
...
legend('光伏实际出力','光伏预测最大出力'); % 在坐标轴上添加图例
xlabel('时刻（t）');
ylabel('功率（kW）');
...
legend('光伏最大出力','风机预测最大出力'); 
xlabel('时刻（t）');
ylabel('功率（kW）');

figure
s = value([Pload]);
plot(s,'r-*','LineWidth',2)  % 修正：改为plot(s)而不是plot(a)
legend('电负荷');
xlabel('时刻（t）');
ylabel('功率（kW）');
...
fprintf('总购电量: %.2f kWh\n', sum(P_buy));
fprintf('总售电量: %.2f kWh\n', sum(P_sell));
fprintf('储能系统总充放电量: %.2f kWh\n', sum(P_ES1_cha)+sum(P_ES1_dis));
