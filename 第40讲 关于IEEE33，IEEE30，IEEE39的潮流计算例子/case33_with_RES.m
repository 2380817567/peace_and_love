function case33_with_RES
% IEEE 33 节点配电系统  +  光伏  +  风机  +  柴油机组

mpc = loadcase('case33bw');     %载入IEEE33节点配电系统算例（需准备case33bw.m）
...
% 按照gen矩阵格式定义光伏机组参数
newgen([GEN_BUS PG QG QMAX QMIN VG MBASE GEN_STATUS PMAX PMIN]) = [6 0.8 0 0.3 -0.3 1.01 100 1 1 0];

mpc.gen = [mpc.gen; newgen];     % 将光伏机组追加到 gen

newcost = zeros(1,ncol_cost);   %初始化一行机组成本参数
newcost(1,1:7) = [2 0 0 3 0.01 10 0];  %定义光伏机组的成本函数
mpc.gencost = [mpc.gencost;newcost]; % 讲光伏机组的成本追加到gencost

%% -----风机（PQ，负负荷建模）---
mpc.bus(18,PD ) = mpc.bus(18,PD ) - 0.5;    %在18号母线上加入风机，相当于减小有功负荷0.5MW

%% --- 柴油机组 ---
newgen = zeros(1,ncol);      %初始化柴油机组数据
% 按照gen矩阵格式定义光伏机组参数
newgen([GEN_BUS PG QG QMAX QMIN VG MBASE GEN_STATUS PMAX PMIN]) = [30 1.5 0 0.5 -0.5 1.00 100 1 2 0];

mpc.gen = [mpc.gen; newgen];     % 将柴油机组追加到 gen

newcost = zeros(1,ncol_cost);   %初始化柴油机组成本参数
newcost(1,1:7) = [2 0 0 3 0.02 40 0];  %定义光伏机组的成本函数
